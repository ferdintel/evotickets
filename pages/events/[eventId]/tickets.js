...
export default function ManageTickets() {
  ...
  // added code - handle download QRCodes
  const downloadQRCodes = () => {
    const tiketCodes = tickets.map((item) => item.get("code"));
  
    const zip = new JSZip();
    tiketCodes.map((code) => {
      let imgData = QR.drawImg(code, {
        typeNumber: 4,
        errorCorrectLevel: "M",
        size: 200,
      });
    
      // delete the “data:image/gif;base64,” part to obtain base64 data only
      const base64Data = imgData.split(",")[1];
  
      zip.file(`${code.slice(0,8)}.png`, base64Data, { base64: true });
    });
  
    zip.generateAsync({ type: "blob" }).then(function (content) {
      saveAs(content, `QRCodes_Project_${eventId}.zip`);
    });
  };
        ...
        <>
          <div className="flex justify-start items-center p-6">
            ...
            <h1 className="text-2xl">
              Gestion de billets pour {event.get("name")}
            </h1>
          </div>
          ...
          <div className="flex items-center justify-between">
            <button
              className={`bg-blue-500 text-white p-2 rounded m-6 mt-0 
            ...
            {/* added code - btn to download QRCodes */}
            <button
              className={`bg-blue-500 text-white p-2 rounded m-6 mt-0 
              ${loading ? "opacity-50 cursor-not-allowed" : ""}`}
              ...
              disabled={loading}
            >
              {loading ? "Génération..." : "Télécharger les QRCodes"}
            </button>
          </div>
                  ...
                  <tr key={ticket.id}>
                    ...
                    <td className="py-2 px-4 border-b">
                      <select
                        ...
                        onChange={(e) =>
                          changeTicketStatus(ticket, e.target.value)
                        }
                      >
